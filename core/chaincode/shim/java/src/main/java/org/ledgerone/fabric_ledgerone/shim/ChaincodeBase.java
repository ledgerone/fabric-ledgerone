...
package org.ledgerone.fabric_ledgerone.shim;

import static org.ledgerone.fabric_ledgerone.shim.Chaincode.Response.Status.INTERNAL_SERVER_ERROR;
import static org.ledgerone.fabric_ledgerone.shim.Chaincode.Response.Status.SUCCESS;
...
import org.ledgerone.fabric_ledgerone.protos.peer.ChaincodeSupportGrpc;
import org.ledgerone.fabric_ledgerone.protos.peer.ChaincodeSupportGrpc.ChaincodeSupportStub;
import org.ledgerone.fabric_ledgerone.shim.impl.Handler;
import org.ledgerone.fabric_ledgerone.shim.impl.NextStateInfo;
...
public abstract class ChaincodeBase implements Chaincode {
	...
	public void chatWithPeer(ManagedChannel connection) {
		...
		try {
			requestObserver = stub.register(new StreamObserver<ChaincodeMessage>() {
				...
				public void onNext(ChaincodeMessage message) {
					...
					try {
						logger.debug(String.format("[%-8s]Received message %s from org.ledgerone.fabric_ledgerone.shim", message.getTxid(), message.getType()));
		...
		// Create the org.ledgerone.fabric_ledgerone.shim handler responsible for all
		// control logic
