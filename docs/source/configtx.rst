...
``fabric-ledgerone/protos/common/configtx.proto``. The Envelope of type
...
in ``fabric-ledgerone/protos/peer/configuration.proto``. The notations
``common.<MSG>``, ``msp.<MSG>``, and ``orderer.<MSG>`` correspond
similarly, but with their messages defined in
``fabric-ledgerone/protos/common/configuration.proto``,
``fabric-ledgerone/protos/msp/mspconfig.proto``, and
``fabric-ledgerone/protos/orderer/configuration.proto`` respectively.
